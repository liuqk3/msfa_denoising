...
class conv_block(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, mask_in_ch, mask_out_ch):
        ...
        self.conv1 = PacConv2d(in_ch, out_ch, kernel_size=3, stride=1, native_impl=True, padding=1, bias=True)
        self.conv2 = PacConv2d(out_ch, out_ch, kernel_size=3, stride=1, native_impl=True, padding=1, bias=True)

        self.act = nn.LeakyReLU(negative_slope=0.01, inplace=True)
        self.mask_transform1 = nn.Conv2d(mask_in_ch, mask_out_ch, kernel_size=3, stride=1, padding=1, bias=True)
        self.mask_transform2 = nn.Conv2d(mask_out_ch, mask_out_ch, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward(self, x, mask):
        ...
        x = self.conv1(x, mask)
        mask = self.mask_transform1(mask)
        x, mask = self.act(x), self.act(mask)
        x = self.conv2(x, mask)
        mask = self.mask_transform2(mask)
        x, mask = self.act(x), self.act(mask)
...
#     net = U_Net_pac(25, 25, 25, ps=False)
#     net.cuda()
