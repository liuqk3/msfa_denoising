...
class DataLoaderTrainSynCB(Dataset):
    ...
    def __getitem__(self, index):
        ...
        noisy, noisy_cb = self.NoiseModel(clean, ratio)
        noisy = np.clip(noisy, 0, 1)
        noisy_cb = np.clip(noisy_cb, 0, 1)
        ...
        clean = clean[:, r:r + ps, c:c + ps]
        noisy = noisy[:, r:r + ps, c:c + ps]
        noisy_cb = noisy_cb[:, r:r+ps, c:c+ps]

        clean = torch.from_numpy(clean)
        noisy = torch.from_numpy(noisy)
        noisy_cb = torch.from_numpy(noisy_cb)

        apply_trans = transforms_aug[random.getrandbits(3)]

        clean = getattr(augment, apply_trans)(clean)
        noisy = getattr(augment, apply_trans)(noisy)    
        noisy_cb = getattr(augment, apply_trans)(noisy_cb)    

        return clean, noisy, noisy_cb


class DataLoaderTrainSynCBParam(Dataset):
    # def __init__(self, rgb_dir, ratio=10, img_options=None, target_transform=None, start=0, end=45):
    def __init__(self, rgb_dir, list_path, ratio=10, noisemodel='PGRC', img_options=None, target_transform=None):
        super(DataLoaderTrainSynCBParam, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        cb_a = self.NoiseModel.color_bias_a[:, np.newaxis, c:c + ps].repeat(ps, axis=1)
        cb_b = self.NoiseModel.color_bias_b[:, np.newaxis, c:c + ps].repeat(ps, axis=1)
        cb_c = self.NoiseModel.color_bias_c[:, np.newaxis, c:c + ps].repeat(ps, axis=1)

        # cb_a = self.NoiseModel.color_bias_a[:, c:c + ps]
        # cb_b = self.NoiseModel.color_bias_b[:, c:c + ps]
        # cb_c = self.NoiseModel.color_bias_c[:, c:c + ps]

        clean = torch.from_numpy(clean)
        noisy = torch.from_numpy(noisy)
        noisy_cb = torch.from_numpy(noisy_cb)
        cb_a = torch.from_numpy(cb_a)
        cb_b = torch.from_numpy(cb_b)
        cb_c = torch.from_numpy(cb_c)
        ...
        noisy = getattr(augment, apply_trans)(noisy)    
        noisy_cb = getattr(augment, apply_trans)(noisy_cb)    
        cb_a = getattr(augment, apply_trans)(cb_a)
        cb_b = getattr(augment, apply_trans)(cb_b)
        cb_c = getattr(augment, apply_trans)(cb_c)

        return clean, noisy, noisy_cb, cb_a, cb_b, cb_c


class DataLoaderTrainSynColIndex(Dataset):
    # def __init__(self, rgb_dir, ratio=10, img_options=None, target_transform=None, start=0, end=45):
    def __init__(self, rgb_dir, list_path, ratio=10, noisemodel='PGRC', img_options=None, target_transform=None):
        super(DataLoaderTrainSynColIndex, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        col_index = np.arange(c,c+ps)
        col_index = col_index[np.newaxis, np.newaxis, :].repeat(ps, axis=1)
        # col_index = col_index / W
        col_index = (col_index + 1) // (W + 2)


        clean = torch.from_numpy(clean)
        noisy = torch.from_numpy(noisy)
        noisy_cb = torch.from_numpy(noisy_cb)
        col_index = torch.from_numpy(col_index)
        ...
        noisy = getattr(augment, apply_trans)(noisy)    
        noisy_cb = getattr(augment, apply_trans)(noisy_cb)    
        col_index = getattr(augment, apply_trans)(col_index)

        return clean, noisy, noisy_cb, col_index
...
class DataLoaderValCBParam(Dataset):
    def __init__(self, rgb_dir, list_path, ratio=10, target_transform=None):
        super(DataLoaderValCBParam, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        if val_or_not:
            ...
            cb_a = self.NoiseModel.color_bias_a[:, np.newaxis, :w].repeat(h, axis=1)
            cb_b = self.NoiseModel.color_bias_b[:, np.newaxis, :w].repeat(h, axis=1)
            cb_c = self.NoiseModel.color_bias_c[:, np.newaxis, :w].repeat(h, axis=1)
        ...
        return clean, noisy, clean_filename, noisy_filename, cb_a, cb_b, cb_c



class DataLoaderValColIndex(Dataset):
    def __init__(self, rgb_dir, list_path, ratio=10, target_transform=None):
        super(DataLoaderValColIndex, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        if val_or_not:
            ...
            col_index = np.arange(0,w)
            col_index = col_index[np.newaxis, np.newaxis, :].repeat(h, axis=1)
            col_index = (col_index + 1) // (W + 2)
        ...
        return clean, noisy, clean_filename, noisy_filename, col_index


class BuildPairedCBData(Dataset):
    # def __init__(self, rgb_dir, ratio=10, img_options=None, target_transform=None, start=0, end=45):
    def __init__(self, rgb_dir, list_path, ratio=10, noisemodel='PGRC', img_options=None, target_transform=None):
        super(BuildPairedCBData, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        clean = np.float32(np.load(self.clean_filenames[tar_index]))
        ...
        return clean, noisy, noisy_cb, self.clean_filenames[tar_index]
        return clean, noisy, self.clean_filenames[tar_index]
...
if __name__ == "__main__":
    ...
    data_dir = '/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/data/scene_data_v4'
    list_dir = '/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/data/dataset_v4/noisy_image.txt'
    save_data_noisy_all = '/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/data/noisy_image'
    # save_data_noisy_cb = '/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/data/syn_data_c'

    os.makedirs(save_data_noisy_all, exist_ok=True)

    # generater = UNetG(25)
    # generater.load_state_dict(torch.load('/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/DANet_ckpt/model_state_130.pt', map_location='cpu')['G'])

    # generater.cuda()
    # os.makedirs(save_data_noisy_cb, exist_ok=True)

    for noisemodel in ['PgRC', 'p', 'pg', 'PgRCbU', 'PgRCBU']:
    # for noisemodel in ['PgRC', 'p', 'pg', 'PgRCbU', 'PgRCBU', 'complex']:
    # for noisemodel in ['g']:
        train_dataset = BuildPairedCBData(data_dir, list_dir, ratio=20, noisemodel=noisemodel)
        # dataloader = DataLoader(train_dataset, batch_size=1)
        # print(ratio)

        # for i, data in enumerate(tqdm(dataloader), 0):  
            # clean = data[0].cuda()
            # noisy = sample_generator(generater, clean)
        ...
        for i in tqdm(range(len(train_dataset))):
            ...
            clean = data[0]
            noisy = data[1]
            # noisy_cb = data[2] /ratio
            file_name = data[3]

            scene = data[3].split('/')[-2]
            Image.fromarray((noisy[2,:192,:384]*255).astype(np.uint8)).save(os.path.join(save_data_noisy_all, scene+'_'+noisemodel+'.png'))

            # import pdb; pdb.set_trace()

            # os.makedirs(file_name[:-len('gt.npy')].replace(data_dir, save_data_noisy_all), exist_ok=True)
            # os.makedirs(file_name[:-len('gt.npy')].replace(data_dir, save_data_noisy_cb), exist_ok=True)

            # np.save(os.path.join(file_name[:-len('gt.npy')].replace(data_dir, save_data_noisy_all), str(int(800/ratio))+'.npy'), noisy)
            # np.save(os.path.join(file_name[:-len('gt.npy')].replace(data_dir, save_data_noisy_all), 'gt.npy'), clean)
            # np.save(os.path.join(file_name[:-len('gt.npy')].replace(data_dir, save_data_noisy_cb), str(int(800/ratio))+'.npy'), noisy_cb)
            # np.save(os.path.join(file_name[:-len('gt.npy')].replace(data_dir, save_data_noisy_cb), 'gt.npy'), clean)
