...
class DataLoaderTrainSynCoord(Dataset):
    def __init__(self, rgb_dir, list_path, ratio=10, noisemodel='PGRC', img_options=None, target_transform=None):
        super(DataLoaderTrainSynCoord, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        return clean, noisy, noisy_cb, (r, c)


class DataLoaderTrainRealColIndex(Dataset):
    def __init__(self, rgb_dir, list_path, ratio=10, img_options=None, target_transform=None):
        super(DataLoaderTrainRealColIndex, self).__init__()
        ...
        if ratio == 'mix':
            ...
            for r in ratio_table:
                self.noisy_filenames[r] = [os.path.join(s, ratio_table[r]+'.npy') for s in scenes]
                self.noisy[r] = [np.float32(np.load(self.noisy_filenames[r][index])) for index in range(len(self.noisy_filenames[r]))]
        else:
            self.noisy_filenames = [os.path.join(s, ratio_table[ratio]+'.npy') for s in scenes]
            self.noisy = [np.float32(np.load(self.noisy_filenames[index])) for index in range(len(self.noisy_filenames))]
    ...
    def __getitem__(self, index):
        ...
        if self.ratio == 'mix':
            ratio = np.random.choice(list(list(self.ratio_table)))
            noisy = self.noisy[ratio][tar_index]
            noisy_filename = os.path.split(self.noisy_filenames[ratio][tar_index])[-1]
        else:
            ratio = self.ratio
...
class DataLoaderVal_all_cb(Dataset):
    def __init__(self, rgb_dir, list_path, ratio=10, target_transform=None):
        super(DataLoaderVal_all_cb, self).__init__()

        self.target_transform = target_transform

        self.data_list = get_data_list(list_path)

        self.noisy_all_path = '/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/data/syn_data_all'
        self.noisy_cb_path = '/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/data/syn_data_cb'
        # ratio_table = {10:'80', 20:'40'}
        ratio_table = {10:'80', 20:'40', 40:'20'}

        scenes = [os.path.join(rgb_dir, d) for d in self.data_list]
        noisy_all_scenes = [os.path.join(self.noisy_all_path, d) for d in self.data_list]
        noisy_cb_scenes = [os.path.join(self.noisy_cb_path, d) for d in self.data_list]
        self.clean_filenames = [os.path.join(s, 'gt.npy') for s in scenes]

        if ratio == 'mix':
            self.noisy_filenames_all = {}
            self.noisy_filenames_cb = {}
            self.noisy_all = {}
            self.noisy_cb = {}
            for r in ratio_table:
                # self.noisy_filenames[r] = [os.path.join(s, ratio_table[r]+'.npy') for s in scenes]
                self.noisy_filenames_all[r] = [os.path.join(s, ratio_table[r]+'.npy') for s in noisy_all_scenes]
                self.noisy_filenames_cb[r] = [os.path.join(s, ratio_table[r]+'.npy') for s in noisy_cb_scenes]
                self.noisy_all[r] = [torch.from_numpy(np.float32(np.load(self.noisy_filenames_all[r][index]))) for index in range(len(self.noisy_filenames_all[r]))]
                self.noisy_cb[r] = [torch.from_numpy(np.float32(np.load(self.noisy_filenames_cb[r][index]))) for index in range(len(self.noisy_filenames_cb[r]))]
        else:
            # self.noisy_filenames = [os.path.join(s, ratio_table[ratio]+'.npy') for s in scenes]
            # self.noisy = [torch.from_numpy(np.float32(np.load(self.noisy_filenames[index]))) for index in range(len(self.noisy_filenames))]
    ...
    def __getitem__(self, index):
        ...
        if self.ratio == 'mix':
            ...
            noisy_all = self.noisy_all[tar_r][tar_index]
            noisy_cb = self.noisy_cb[tar_r][tar_index]
            # noisy_filename = os.path.split(self.noisy_filenames[tar_r][tar_index])[-1]
        ...
        w = clean.shape[2]//16*16
        clean = clean[:, -h:, :w]
        noisy_all = noisy_all[:, -h:, :w] * tar_r
        noisy_cb = noisy_cb[:, -h:, :w] * tar_r
        
        noisy_all = np.clip(noisy_all, 0, 1)
        noisy_cb = np.clip(noisy_cb, 0, 1)

        return clean, noisy_all, noisy_cb


##################################################################################################
class DataLoaderTrainRealCoord(Dataset):
    # def __init__(self, rgb_dir, ratio=10, img_options=None, target_transform=None, start=0, end=45):
    def __init__(self, rgb_dir, list_path, ratio=10, img_options=None, target_transform=None):
        super(DataLoaderTrainRealCoord, self).__init__()
