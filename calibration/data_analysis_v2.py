...
def refine_plot(ax, r=None, fontsize=28):
    ...
    if r is not None:
        plt.annotate('$R^2 = {:.3f}$'.format(r**2), xy=(0.5, 0.1), xycoords='axes fraction', fontsize=fontsize)
...
def remove_cb(img):
    ...
    img = img - y_fitted_q.reshape((img.shape[0],1))
...
def BF_ISO_analysis(k=128):
    ...
    refine_plot(ax, r)
    # plt.show()
    plt.savefig('figure_withcb_test/BF_analysis/band_{}_row.png'.format(k), bbox_inches='tight')
    plt.clf()
    ...
    refine_plot(ax, r)
    # plt.show()
    plt.savefig('figure_withcb_test/BF_analysis/band_{}_col.png'.format(k), bbox_inches='tight')
    plt.clf()
    ...
    refine_plot(ax, r, fontsize=28)

    # plt.show()
    plt.savefig('figure_withcb_test/BF_analysis/band_{}_read_gauss.png'.format(k), bbox_inches='tight')        
    plt.clf()
    ...
    # plt.show()
    plt.savefig('figure_withcb_test/BF_analysis/band_{}_read_PPCC.png'.format(k), bbox_inches='tight')        
    plt.clf()
    ...
    # plt.show()
    plt.savefig('figure_withcb_test/BF_analysis/band_{}_read_TL.png'.format(k), bbox_inches='tight')        
    plt.clf()
...
def banding_FFT(frame, k=128):
    ...
    # plt.show() 
    plt.savefig('figure_withcb_test/BF_analysis/band_{}_FFT.png'.format(k), bbox_inches='tight')        
    plt.clf()
...
if __name__ == '__main__':
    if not os.path.exists('/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/calibration/figure_withcb_test/BF_analysis'):
        os.makedirs('/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/calibration/figure_withcb_test/BF_analysis')
    for k in range(25):
        # FF_analysis(k)
        BF_ISO_analysis(k)
