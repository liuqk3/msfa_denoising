...
def BF_ISO_analysis(k=128):
    ...
    refine_plot(ax, r)
    # plt.show()
    plt.savefig('figure_withcb/BF_analysis/band_{}_row.pdf'.format(k), bbox_inches='tight')
    plt.clf()
    ...
    refine_plot(ax, r)
    # plt.show()
    plt.savefig('figure_withcb/BF_analysis/band_{}_col.pdf'.format(k), bbox_inches='tight')
    plt.clf()
    ...
    # plt.show()
    plt.savefig('figure_withcb/BF_analysis/band_{}_read_gauss.pdf'.format(k), bbox_inches='tight')        
    plt.clf()
    ...
    # plt.show()
    plt.savefig('figure_withcb/BF_analysis/band_{}_read_PPCC.pdf'.format(k), bbox_inches='tight')        
    plt.clf()
    ...
    # plt.show()
    plt.savefig('figure_withcb/BF_analysis/band_{}_read_TL.pdf'.format(k), bbox_inches='tight')        
    plt.clf()
...
def banding_FFT(frame, k=128):
    ...
    # plt.show() 
    plt.savefig('figure_withcb/BF_analysis/band_{}_FFT.pdf'.format(k), bbox_inches='tight')        
    plt.clf()
...
def FF_analysis(k, flat_frame_1, flat_frame_2):
    ...
    refine_plot(ax)
    plt.savefig('figure/FF_analysis/band_{}.pdf'.format(k), bbox_inches='tight')
    print('Plot saved in band_{}.png'.format(k))
    plt.clf()
    return slope, slope_se

if __name__ == '__main__':
    if not os.path.exists('/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/calibration/figure_withcb/BF_analysis'):
        os.makedirs('/home/jiangyuqi/Desktop/HSI_denoising_demosaicing/calibration/figure_withcb/BF_analysis')
    for k in range(25):
        # FF_analysis(k)
        BF_ISO_analysis(k)
